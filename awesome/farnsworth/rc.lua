...
function volume (mode, widget)
	local command = ".config/awesome/vol.sh"
	if mode == "up" then
		command = command .. " up 1"
	elseif mode == "down" then
		command = command .. " down 1"
	end
        
	local volume = io.popen(command):read("*all")
	volume = volume.gsub(volume, "\n", "")
...
mytimer:add_signal("timeout", function() volume("update", tb_volume) end)
mytimer:start()
